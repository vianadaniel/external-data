...
export class LoggingInterceptor implements NestInterceptor {
  ...
  private determineNodeEnv(request: Request): string {
    ...
    // Normalizar IP: remover prefixo IPv6 mapeado (::ffff:) se existir
    const normalizedIp = ipString.replace(/^::ffff:/, '');

    // Verificar se o IP é 98.86.185.72 (produção)
    // Considera tanto o formato IPv4 direto quanto o formato IPv6 mapeado (::ffff:98.86.185.72)
    if (normalizedIp === '98.86.185.72' || ipString === '98.86.185.72') {
      return 'prod';
    }
